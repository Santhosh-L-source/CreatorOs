...
document.addEventListener('DOMContentLoaded', () => {
    ...
    function renderGanttView() {
        ...
        const paddingMs = timelineRange * 0.05;
        const timelineStart = new Date(minDate.getTime() - paddingMs);
        const timelineEnd = new Date(maxDate.getTime() + paddingMs);
        const totalDuration = timelineEnd - timelineStart;

        // Add timeline scale header
        const scaleRow = document.createElement('div');
        scaleRow.className = 'gantt-scale-row';
        scaleRow.style.cssText = 'display: grid; grid-template-columns: 220px 1fr; align-items: center; margin-bottom: 10px; font-size: 0.85rem;';
        ...
        scaleRow.innerHTML = `
            <div style="padding: 0.5rem; font-weight: bold;">Tasks</div>
            <div style="position: relative; height: 30px; background: #f9fafb; border: 1px solid #e5e7eb; display: flex; align-items: center; padding: 0 10px; justify-content: space-between;">
                <span style="font-size: 0.75rem;">${formatDate(timelineStart)}</span>
                <span style="font-size: 0.75rem; position: absolute; left: 50%; transform: translateX(-50%);">${formatDate(new Date((timelineStart.getTime() + timelineEnd.getTime()) / 2))}</span>
                <span style="font-size: 0.75rem;">${formatDate(timelineEnd)}</span>
            </div>
        `;
        ...
        activeTasks.forEach(task => {
            ...
            if (dueDate <= startDate) {
                dueDate = new Date(startDate.getTime() + 86400000); // Add 1 day minimum duration
            }

            // Calculate position and width as percentages
            const leftPercent = Math.max(0, Math.min(100, ((startDate - timelineStart) / totalDuration) * 100));
            const taskDuration = dueDate - startDate;
            let widthPercent = Math.max(2, Math.min(100 - leftPercent, (taskDuration / totalDuration) * 100));

            // Ensure minimum visible width for single-day tasks
            if (widthPercent < 2) widthPercent = 2;

            // Format dates for tooltip
            const dateTooltip = `Start: ${formatDate(startDate)} | Due: ${formatDate(dueDate)}`;

            row.innerHTML = `
                <div class="gantt-label" title="${escapeHtml(task.title)}">${escapeHtml(task.title)}</div>
                <div class="gantt-track">
                    <div class="gantt-bar ${task.status} ${task.priority}"
